...
class DateProcessor:
    ...
    def find_dates(text):
        ...
        dates = []
        dates.extend(
            DateProcessor.__find_dates(text, DateProcessor.DATE_REGEX_1, "%d %b %Y")
        )
        if len(dates) == 0:
            dates.extend(
                DateProcessor.__find_dates(text, DateProcessor.DATE_REGEX_2, "%d %b %y")
            )
        dates.extend(
            DateProcessor.__find_dates(text, DateProcessor.DATE_REGEX_3, "%m/%d/%y")
        )
        dates.extend(
            DateProcessor.__find_dates(text, DateProcessor.DATE_REGEX_3, "%d/%m/%y")
        )
        dates.extend(
            DateProcessor.__find_dates(text, DateProcessor.DATE_REGEX_4, "%m/%d/%Y")
        )
        dates.extend(
            DateProcessor.__find_dates(text, DateProcessor.DATE_REGEX_4, "%d/%m/%Y")
        )
        dates.extend(
            DateProcessor.__find_dates(text, DateProcessor.DATE_REGEX_5, "%b %d, %Y")
        )
    ...
    def __find_dates(text, date_regex, datetime_format):
        ...
        datetime_objects = list(
            map(lambda d: DateProcessor.__format_date(d, datetime_format), date_matches)
        )
